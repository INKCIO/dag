...
pub type INKCConn = Actor<WsConnection<TcpStream>>;

lazy_static! {
    pub static ref INBOUND_CONN: RwLock<Vec<INKCConn>> = RwLock::new(Vec::new());
    pub static ref OUTBOUND_CONN: RwLock<Vec<INKCConn>> = RwLock::new(Vec::new());
}
...
pub fn new_ws<A: ToSocketAddrs>(address: A) -> Result<INKCConn> {
